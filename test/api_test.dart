...
void main() {
  group('VirusTotalService API Status Code Tests', () {
    test('200 OK: scanUrl successfully submits URL and parses analysis id', () async {
      ...
      final service = VirusTotalService(client: mockClient, apiKey: 'test-vt-key');
    ...
    test('200 OK: getAnalysis returns completed analysis model', () async {
      ...
      expect(analysis.data.attributes.results['Google']?.result, equals('clean'));
    });

    test('400 Bad Request: throws VirusTotalBadRequestException', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
          jsonEncode({'error': {'message': 'Invalid URL format'}}),
          400,
        );
      });
    ...
    test('401 Unauthorized: throws VirusTotalAuthException', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
          jsonEncode({'error': {'message': 'Wrong API key'}}),
          401,
        );
      });

      final service = VirusTotalService(client: mockClient, apiKey: 'invalid-key');
      expect(
    ...
    test('429 Rate Limit Exceeded: throws VirusTotalRateLimitException', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
          jsonEncode({'error': {'message': 'Quota exceeded'}}),
          429,
        );
      });
  ...
  group('TatumService API Status Code Tests', () {
    ...
    test('200 OK: getCryptoWalletAnalysis parses native balance, assets & safety', () async {
      final mockClient = MockClient((request) async {
        expect(request.headers['x-api-key'], equals('test-tatum-key'));
        if (request.url.path.contains('/v4/data/blockchains/balance')) {
          return http.Response(
            jsonEncode({'balance': '3.75', 'incoming': '10', 'outgoing': '6.25'}),
            200,
          );
      ...
      final service = TatumService(client: mockClient, apiKey: 'test-tatum-key');
    ...
    test('400 Bad Request: checkMaliciousAddress safely handles 400 error', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
    ...
    test('401 Unauthorized: checkMaliciousAddress fallback returns unknown status', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
      ...
      final service = TatumService(client: mockClient, apiKey: 'invalid-tatum-key');
      final safety = await service.checkMaliciousAddress(testWallet);
      expect(safety.status, equals(MaliciousCheckStatus.unknown));
    });

    test('403 Forbidden: checkMaliciousAddress fallback returns unknown status', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
    ...
    test('429 Rate Limit: checkMaliciousAddress fallback returns unknown status', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
    ...
    test('500 Server Error: checkMaliciousAddress fallback returns unknown status', () async {
      final mockClient = MockClient((request) async {
        return http.Response('Internal Server Error', 500);
      });
